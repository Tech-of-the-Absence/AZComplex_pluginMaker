...
@PluginComponent
public class TESTAction implements ActionType{ //Pay attention to the name format. The pattern is YOURNAMEAction
	@Override public Consumer<ITaskExecutor>getAction(ActionCommand c){
		...
		return f->{
			...
			f.coordinators().get(c.coords).init(f); //Coordinator initialization (should be removed if action is not positional)
			Point coords;
			int x=0,y=0;
	...
	@Override public void paint(Graphics2D g2,int x,int y,int s/*radius*/){
